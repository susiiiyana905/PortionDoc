...
router.post("/add/grocery", auth.verifyAdmin, upload.single('groceryImage'), async(req,res)=>{
    ...
    groceryData.save()
    .then(function(){
        res.status(200).send({success: true,data:groceryData, message: "New grocery added successfully!"});
    }).catch(function(e){
...
router.put("/update/grocery/:gid", auth.verifyAdmin, upload.single("groceryImage"), async(req,res)=>{
    ...
    .then((groceryData)=>{
        ...
        .then(function(){
            res.status(200).send({success:true, message: "Grocery details has been updated!"})
        })
        .catch(function(){
            res.status(400).send({message: e})
        });
...
router.get('/all/grocery', auth.verifyUser, async(req,res)=>{
    const GroceryData = await Grocery.find()
    res.json({success: true, message:"Meals Data", data:GroceryData});
})
...
router.delete("/grocery/delete/:gid", auth.verifyAdmin, function(req,res){
    ...
    .then(function(){
        res.status(200).send({success:true, message: "Grocery has been deleted!"});
    })
    .catch(function(){
        res.status(400).send({message: "Something went wrong!"});
    })
